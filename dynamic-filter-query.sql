...
with all_country_entries as(
  ...
  sum(ifnull(bid_opportunities,0)) requests,
  sum(ifnull(estimated_adjusted_revenue,0)) rev,
  safe_divide(sum(ifnull(estimated_adjusted_revenue,0)),sum(ifnull(bid_opportunities,0)))*1000 rpr,
  safe_divide(sum(ifnull(bids,0)),sum(ifnull(bid_opportunities,0)))*100 bid_rate,
  sum(ifnull(case when error_code is null then total_top_bids end,0)) total_top_bids,
  round(safe_divide(sum(ifnull(case when error_code is null then total_top_bids end,0)),sum(ifnull(bid_requests,0)))*100,2) top_bid_rate,
  case when sum(ifnull(bid_opportunities,0))>min_requests then 'high' else 'low' end request_volume,
...
country_entries_treated as (
  ...
  sum(ifnull(a.bid_opportunities,0)) requests,
  sum(ifnull(a.estimated_adjusted_revenue,0)) rev,
  safe_divide(sum(ifnull(estimated_adjusted_revenue,0)),sum(ifnull(bid_opportunities,0)))*1000 rpr,
  safe_divide(sum(ifnull(a.bids,0)),sum(ifnull(a.bid_opportunities,0)))*100 bid_rate,
  sum(ifnull(case when error_code is null then a.total_top_bids end,0)) total_top_bids,
  round(safe_divide(sum(ifnull(case when error_code is null then a.total_top_bids end,0)),sum(ifnull(bid_requests,0)))*100,2) top_bid_rate,
...
host_entries as(SELECT bidder_id bidder, geo_continent continent,geo_country country,host,
  ...
  AND bidder_id IS NOT NULL
  group by 1,2,3,4
  having sum(ifnull(bid_opportunities,0))>=min_requests
  ),
